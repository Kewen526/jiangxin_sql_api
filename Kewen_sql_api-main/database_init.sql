-- ================================================
-- Jiangxin SQL API 平台 - 完整数据库初始化脚本
-- ================================================
-- 执行说明：
...
-- ================================================
-- 第一部分：清理旧数据（Kewen_sql_api）
-- ================================================

SET FOREIGN_KEY_CHECKS = 0;

-- 删除旧的API配置
DELETE FROM apis WHERE tenant_id IN (
    SELECT id FROM tenants WHERE name LIKE '%Kewen%' OR name LIKE '%kewen%'
);

-- 删除旧的数据源
DELETE FROM datasources WHERE tenant_id IN (
    SELECT id FROM tenants WHERE name LIKE '%Kewen%' OR name LIKE '%kewen%'
);

-- 删除旧的API分组
DELETE FROM api_groups WHERE tenant_id IN (
    SELECT id FROM tenants WHERE name LIKE '%Kewen%' OR name LIKE '%kewen%'
);
...
-- 删除旧的租户
DELETE FROM tenants WHERE name LIKE '%Kewen%' OR name LIKE '%kewen%';
...
GROUP BY a.id, a.tenant_id, a.name, a.path, a.method;
...
ON DUPLICATE KEY UPDATE name = '系统';
...
ON DUPLICATE KEY UPDATE email = 'admin@jiangxin.com';
...
VALUES (
    ...
    'SYSTEM',
    '系统-用户管理',
    '预置的用户和权限管理API，供所有租户使用',
    0,
    NOW()
)
ON DUPLICATE KEY UPDATE name = '系统-用户管理';
...
INSERT INTO apis (
    ...
    JSON_OBJECT(
        ...
        'sql', 'SELECT id, tenant_id, email, role, status, created_at, updated_at FROM users WHERE tenant_id = #{tenant_id} AND status = 1 ORDER BY created_at DESC LIMIT #{limit} OFFSET #{offset}'
    ),
    JSON_OBJECT('tenant_id', 'test-tenant-001', 'page', 1, 'page_size', 20),
    1,
    NOW()
)
ON DUPLICATE KEY UPDATE name = '获取用户列表';
...
INSERT INTO apis (
    ...
    JSON_ARRAY(
        ...
        JSON_OBJECT('name', 'tenant_id', 'type', 'string', 'required', true, 'description', '租户ID（权限验证）')
    ),
    JSON_OBJECT(
        'datasource_id', 'PLATFORM',
...
ON DUPLICATE KEY UPDATE name = '获取用户详情';

-- ================================================
-- 预置API 3: 创建用户（仅SQL查询部分，实际创建需要密码加密）
-- ================================================
INSERT INTO apis (
    id, tenant_id, group_id, name, path, method, note,
    content_type, params, sql_config, test_params, status, created_at
...
ON DUPLICATE KEY UPDATE name = '检查邮箱是否存在';
...
ON DUPLICATE KEY UPDATE name = '更新用户状态';
...
ON DUPLICATE KEY UPDATE name = '更新用户角色';
...
ON DUPLICATE KEY UPDATE name = '删除用户';
...
ON DUPLICATE KEY UPDATE name = '获取租户信息';
...
ON DUPLICATE KEY UPDATE name = '用户统计';
...
INSERT INTO apis (
    ...
    JSON_OBJECT(
        'datasource_id', 'PLATFORM',
        'sql', 'SELECT id, tenant_id, email, role, status, created_at FROM users WHERE tenant_id = #{tenant_id} AND (email LIKE CONCAT("%", #{keyword}, "%") OR #{keyword} IS NULL) AND (role = #{role} OR #{role} IS NULL) ORDER BY created_at DESC LIMIT 50'
    ),
    JSON_OBJECT('tenant_id', 'test-tenant-001', 'keyword', 'admin', 'role', 'admin'),
    1,
    NOW()
)
ON DUPLICATE KEY UPDATE name = '搜索用户';
...
ON DUPLICATE KEY UPDATE name = '验证用户权限';
...
SELECT CONCAT('系统租户ID: SYSTEM') as info;
SELECT CONCAT('系统管理员: admin@jiangxin.com') as info;
SELECT CONCAT('初始密码: admin123（请立即修改）') as info;
